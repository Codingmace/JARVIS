...
#                print("That is an invalid link that I am going to ignore")
    ...
    oneTime = oneTimeResponse.lower() == "yes"
    noReply = noReplyResponse.lower() == "yes"
    counter = 0
    newSet = []
    for s in siteList:
        try:
            if (("noreply" in s.getSender().lower()) and noReply:
                for mes in s.messages:
                    service.users().messages().trash(userId='me',id=mes.thread).execute() #trashing thread
                #trashing(s.thread)
